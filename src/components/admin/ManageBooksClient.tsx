...
export function ManageBooksClient({
  ...
  const filtered = books.filter((book: any) => {
    ...
    const authorName = book.author ? t(book.author.name, locale).toLowerCase() : "";
    const matchesSearch =
      !search || title.includes(search.toLowerCase()) || authorName.includes(search.toLowerCase());
    const matchesCategory = !filterCategory || book.category_id === filterCategory;
  ...
  async function handleDelete() {
    ...
    try {
      ...
      if (res.ok) {
        setBooks((prev: any[]) => prev.filter((b: any) => b.id !== deleteTarget.id));
      }
    } catch {
      // Error handling
    } finally {
  ...
  async function handleToggleFeatured(book: any) {
    try {
      ...
      if (res.ok) {
        setBooks((prev: any[]) =>
          prev.map((b: any) =>
            b.id === book.id ? { ...b, is_featured: !b.is_featured } : b
          )
        );
      }
    } catch {
  ...
  async function handleEditSave(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const updatedAuthor = authors.find((a: any) => a.id === payload.author_id);
        const updatedCategory = categories.find((c: any) => c.id === payload.category_id);
        setBooks((prev: any[]) =>
          prev.map((b: any) =>
            b.id === editTarget.id
              ? { ...b, ...data, author: updatedAuthor || b.author, category: updatedCategory || b.category }
              : b
          )
        );
        setEditTarget(null);
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[var(--color-border)] bg-[var(--color-bg)]">
                <th className="text-start px-4 py-3 font-semibold">Book</th>
                <th className="text-start px-4 py-3 font-semibold hidden md:table-cell">Author</th>
                <th className="text-start px-4 py-3 font-semibold hidden lg:table-cell">Category</th>
                <th className="text-center px-4 py-3 font-semibold hidden sm:table-cell">Views</th>
                <th className="text-center px-4 py-3 font-semibold hidden sm:table-cell">Downloads</th>
            ...
            <tbody className="divide-y divide-[var(--color-border)]">
              {filtered.length === 0 ? (
                ...
                filtered.map((book: any) => (
                  <tr key={book.id} className="hover:bg-[var(--color-bg)] transition-colors">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-[var(--color-text-muted)] uppercase">
                            {book.language_code} • {book.copyright?.replace("_", " ")}
                          </p>
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-3 hidden md:table-cell text-[var(--color-text-muted)]">
                      {book.author ? t(book.author.name, locale) : "—"}
                    </td>
                    <td className="px-4 py-3 hidden lg:table-cell">
                      <span className="inline-block px-2 py-0.5 rounded-full text-xs bg-[var(--color-primary)]/10 text-[var(--color-primary)] font-medium">
                        {book.category ? t(book.category.name, locale) : "—"}
                      </span>
                    </td>
      ...
      >
        {editTarget && (
          <form onSubmit={handleEditSave} className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {(["en", "ar", "bn", "ur"] as const).map((lang) => (
                  <div key={lang}>
                    ...
                    <input
                      name={`title_${lang}`}
                      defaultValue={editTarget.title?.[lang] || ""}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="text-sm font-semibold block mb-1">Language</label>
                <select
                  name="language_code"
              ...
              <div>
                <label className="text-sm font-semibold block mb-1">Copyright</label>
                <select
                  name="copyright"
            ...
            <div>
              <label className="text-sm font-semibold block mb-1">Description (English)</label>
              <textarea
                name="description_en"
                rows={3}
