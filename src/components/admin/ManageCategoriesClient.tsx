      ...
      if (res.ok) {
        ...
        if (isEdit) {
          setCategories((prev: any[]) =>
            prev.map((c: any) => (c.id === editTarget.id ? data : c))
          );
          setEditTarget(null);
        } else {
  ...
  function CategoryForm({ category }: { category?: any }) {
    return (
      <form onSubmit={handleCreateOrEdit} className="space-y-4">
        ...
        <fieldset className="space-y-3">
          <legend className="text-sm font-semibold">Description (Multilingual)</legend>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {(["en", "ar", "bn", "ur"] as const).map((lang) => (
              <div key={lang}>
                <label className="text-xs text-[var(--color-text-muted)] uppercase">
                ...
                <input
                  ...
                  defaultValue={category?.description?.[lang] || ""}
                  dir={lang === "ar" || lang === "ur" ? "rtl" : "ltr"}
                  className="w-full mt-1 rounded-lg border border-[var(--color-border)] px-3 py-2 text-sm focus:ring-2 focus:ring-[var(--color-primary)] focus:outline-none"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="text-sm font-semibold block mb-1">Parent Category</label>
            <select
              name="parent_id"
          ...
          <div>
            <label className="text-sm font-semibold block mb-1">Sort Order</label>
            <input
              name="sort_order"
              type="number"
              defaultValue={category?.sort_order ?? categories.length}
              className="w-full rounded-lg border border-[var(--color-border)] px-3 py-2 text-sm focus:ring-2 focus:ring-[var(--color-primary)] focus:outline-none"
            />
          </div>
        </div>
        ...
        <div>
          <label className="text-sm font-semibold block mb-1">Icon URL</label>
          <input
            name="icon_url"
